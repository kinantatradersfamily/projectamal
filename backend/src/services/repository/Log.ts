...
export async function DBCreateActivityLog({ action, url, user_id, params }: LogDto.CreatePayload) {
    const values = [
        [action, url, user_id, params]
    ]

    const query = await db.query<ResultSetHeader>(`
        INSERT INTO activity_log (action, url, user_id, params) VALUES ?
    `, [values])
