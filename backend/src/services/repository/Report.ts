...
export async function DBCreateReport({ date, description, event = 1, pemateri, topic, total_amal, total_attendance, manager_id }: ReportDto.CreatePayload) {
    const values = [
        [date, description, event, pemateri, topic, total_amal, total_attendance, manager_id]
    ]

    const query = await db.query<ResultSetHeader>(`
        INSERT INTO report_attendance (date, description, event, pemateri, topic, total_amal, total_attendance, manager_id) VALUES ?
    `, [values])
...
export async function DBGetReportById(id: number) {
    const query = await db.query<ReportDto.Report[]>(`SELECT id, topic, pemateri, description, event, total_amal, total_attendance, date, manager_id, created_at, updated_at FROM report_attendance`, [id])
    return query
}
